...
A personal Claude Code toolkit — slash commands, skills, scripts, and reference material for building features with Claude Code.

> **Using these assets in a project:** Copy the relevant files from `commands/`, `scripts/`, and `_specs/` into your project's `.claude/` folder. See [Setup](#setup) below.
...
|---|---|---|
| Slash commands | `commands/` | Drop into `.claude/commands/` to register as `/command-name` |
| Skills | `skills/` | Loaded via the `/skill` mechanism to extend Claude's behavior |
| Spec template | `_specs/template.md` | Base structure for every spec generated by `/spec` |
| Scripts | `scripts/` | Shell helpers used by commands (e.g. `git-staged.sh`) |
| Saved prompts | `prompts.md` | Quick-reference prompts and slash command notes |
| Baseline permissions | `settings.local.json` | Starter permissions template — copy and trim to fit the project |
| Global preferences | `CLAUDE.global.md` | Bryan's personal Claude Code preferences (mirrored from `~/.claude/CLAUDE.md`) |
...
Skills live under `skills/` organized by category. Each has a `SKILL.md` entry point.

### Engineering

| Skill | What it does |
|---|---|
| `grill-with-docs` | Stress-tests a plan against the project's domain model — sharpens terminology, cross-references code, updates `CONTEXT.md` / ADRs inline |
| `improve-codebase-architecture` | Surfaces shallow modules, tight coupling, and testability gaps, then guides refactoring using consistent domain language |
| `tdd` | Strict red-green-refactor via vertical tracer bullets — one behavior at a time, through public interfaces only |
| `to-issues` | Converts a plan, spec, or PRD into independently-grabbable vertical-slice issues with dependency ordering and HITL/AFK classification |
| `to-prd` | Synthesizes conversation context and codebase understanding into a structured PRD and publishes it to the issue tracker |

### Productivity

| Skill | What it does |
|---|---|
| `grill-me` | Relentless Socratic interrogation of a plan — walks every branch of the decision tree one question at a time |

---

## Setup

Before using commands in a project for the first time:

```bash
# Create the target directories
mkdir -p your-project/.claude/commands your-project/.claude/scripts your-project/_specs
...
`/commit-message` supports multi-repo setups. The underlying `scripts/git-staged.sh` scans up to two directory levels deep for repos with staged changes when not invoked from a git root.
